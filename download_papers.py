...
def text_from_pdf(pdf_path, temp_path):
    if os.path.exists(temp_path):
        os.remove(temp_path)
    subprocess.call(["/usr/local/bin/pdftotext", pdf_path, temp_path])
...
for year in sorted(index_urls.keys()):
    ...
    for link in paper_links:
        ...
        papers.append([paper_id, paper_title, abstract])
...
pd.DataFrame(papers, columns=["id","title", "abstract"]).sort_values(by="id").to_csv("output/papers.csv", index=False)
